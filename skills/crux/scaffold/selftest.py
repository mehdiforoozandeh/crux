...
def run_auto_grammar():
    ...
    try:
        ...
        # Amended by 05.5. The old assertion was:
        #     "\n- [ ] <<verifiable>>\n" in template  and  "<= <<bar>>" not in template
        # PRD 05.5 section D ordered that bare stub line replaced by a worked Verifiables
        # grammar, so the first literal is superseded — the test contradicted the signed-off
        # requirement, so the test moved. The name and the intent are unchanged: no
        # hard-coded direction in the shipped template. The new assertion is WIDER than the
        # old one, not narrower: instead of the single literal "<= <<bar>>" anywhere in the
        # file, NO placeholder line ("- [ ] ...") inside ## Verifiables may carry ANY
        # comparison operator. The old literal clause is kept as well.
        # Placeholder spans (<<...>>) are stripped before the match, so the angle brackets
        # of <<bar-op>> are not read as operators. The legend beneath the block may — and
        # must — name <= and >=; that legend is what teaches the derivation from direction::.
        # Newlines are normalised first: CI runs a Windows job and the file may read CRLF.
        _tpl_fp = read(os.path.join(HERE, "templates",
                                    "flight_plan.md")).replace("\r\n", "\n")
        _fp_verif = [s for s in re.split(r"(?m)^## ", _tpl_fp)
                     if s.startswith("Verifiables")]
        _fp_boxes = [ln for s in _fp_verif for ln in s.split("\n")
                     if ln.startswith("- [ ]")]
        _fp_ops = [ln for ln in _fp_boxes
                   if re.match(r"^- \[ \].*(<=|>=|<|>|==|!=)",
                               re.sub(r"<<[^<>]*>>", "", ln))]
