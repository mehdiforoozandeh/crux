...
type: idea
schema: 2
title: prefetch lowers latency and reduces memory pressure
parent: q1
status: idea
rule:
measurement:
...
# h2 — prefetch lowers latency and reduces memory pressure

Parent:: [[q1_how_should_the_drafts_be_reviewed]]
...
prefetch lowers latency and reduces memory pressure

## Verifiables

<!-- on close, tick each box met/unmet/could-not-evaluate; the verdict is derived from them. -->
- [ ] cold-start latency falls by >= 20 percent
      fails-if:: cold-start latency is unchanged with prefetch on
- [ ] peak resident memory falls by >= 10 percent
      fails-if:: peak resident memory is unchanged with prefetch on
